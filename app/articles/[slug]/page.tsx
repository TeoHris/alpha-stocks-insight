...
export default async function ArticlePage({
  ...
  // Strip the legacy "TICKER • Date • X min read" sub-header line that older
  // articles have as their first content line (no longer generated by the prompts).
  const cleanedContent = article.content
    .split('\n')
    .filter(line => !/^[\w]+\s*•\s*\w+ \d+,\s*\d{4}\s*•\s*\d+ min read/.test(line.trim()))
    .join('\n')
    .trimStart()
